...
get_vals() {
    ...
    tmp_output=`top -b -n1 -c | grep "$process" | grep -v $0`
    ...
    ps_user=`echo ${tmp_output} | awk '{print $2}'`
    ps_pid=`echo ${tmp_output} | awk '{print $1}' `
    ps_cpu=`echo ${tmp_output} | awk '{print $9}'`
    ps_mem=`echo ${tmp_output} | awk '{print $10}' `
    ps_start=`echo ${tmp_output} | awk '{print $9}' `

    tmp_ps_cputime=`echo ${tmp_output} | awk '{print $11}'`
    tmp_ps_cpuhours=`echo ${tmp_ps_cputime} | awk -F \: '{print $1}'`
    tmp_ps_cpumin=`echo ${tmp_ps_cputime} | awk -F \: '{print $2}' | awk -F\. '{print $1}'`
...
do_wccalc() {
    ...
    then
        if [ "$target" = "cpu" ]
        then
            tmp_wc_target=`echo ${ps_cpu}`
            if [ "$tmp_wc_target" -ge 5 ]
            then
                wc_target=`echo ${ps_cpu}`
