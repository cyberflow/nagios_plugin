...
sub get_tenant_id {
    my ($url) = @_;
    my $tenant_id = _cache()->get('tenant_id');
    unless ($tenant_id) {
	my $data = from_json(_post(_url($url, ":5000/v2.0/tokens"), {auth => { tenantName => $tenant, passwordCredentials => { username => $user, password => $passwd }}})->content);
	$tenant_id = $data->{access}{token}{tenant}{id};
	_cache()->set('tenant_id', $tenant_id, '86400 sec');
    }
    verbose ("GET_TENANT_ID: Tenant = $tenant_id\n", 3);
    return $tenant_id;
}

sub get_services {
    my ($url, $aurl) = @_;
    my $res = _get(_url($url, ":8774/v2/" . get_tenant_id($aurl) . "/os-services"), $aurl);    
    return $res->content;
}
...
sub run {
     $plugin = Nagios::Plugin->new( shortname => 'CHECK_NOVA_API' );
...
EOT
	...
	default  => '5000',
     ...
     #print Dumper(from_json(get_service($options->host, $authurl)));

     my @services = from_json(get_services($options->host, $authurl))->{'services'};     
     my $compute_services = 0;
     my $alive = 0;
     foreach my $s ( @{$services[0]} ) {
	 #print Dumper($s);
     	 if ($s->{'binary'} eq 'nova-compute') {
     	     $compute_services += 1;
     	     if ($s->{'state'} eq 'up') {
     	 	 $alive += 1;
     	     }
     ...
     if ($compute_services == 0) {
     	 $plugin->nagios_exit( 'CRITICAL', "Services were obtained." );
     } elsif ($alive == 0) {
     	 $plugin->nagios_exit( 'CRITICAL', "All services are not be up." );
     } else {
